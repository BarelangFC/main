...
public:
        ...
        : Node("main_strategy")
    {
        ...
        factory.registerSimpleAction("getDataWalk", std::bind(&main_strategy::getDataWalk, this));
        factory.registerSimpleAction("Kiper", std::bind(&main_strategy::Kiper, this));
...
private:
    ...
    NodeStatus Attack()
    {
        /* if (rst_mov_attack > 5 && done_generated)
        {
    ...
    NodeStatus Kiper()
    {
        ...
        {
            case 0:
                    // int Grid_Pos = coordinates_to_grid(-200, 50);
                    // std::cout << "Grid position: " << Grid_Pos << std::endl;  

                    // Grid_Pos = coordinates_to_grid(-200, -50);
                    // std::cout << "Grid position: " << Grid_Pos << std::endl;  
                    
                    // Grid_Pos = coordinates_to_grid(-200, 150);
                    // std::cout << "Grid position: " << Grid_Pos << std::endl;  

                    // Grid_Pos = coordinates_to_grid(-200, -150);
                    // std::cout << "Grid position: " << Grid_Pos << std::endl;

                    // Grid_Pos = coordinates_to_grid(-200, 250);
                    // std::cout << "Grid position: " << Grid_Pos << std::endl;  
            ...
            case 1:   //searchball
                    ...
                    if(ballLost(20)){
                        threeSearchBall();
                        printf("Case 1 Cari bola \n");
                    }else{
                         printf("Case 1 bola dapat \n");
                        trackBall();
                        if(tunggu > 20){
                            stateCondition = 2;
                        }else{
    ...
    NodeStatus GetData()
    {
        if (!doneGetData)
        {
            if (cnt_get_data <= 20)
            {
                ...
                std::mt19937 gen(rd());
                std::uniform_real_distribution<> dis(-1.6, -0.8);
                float tiltMove = dis(gen);
                std::random_device rd_;
                std::mt19937 gen_(rd_());
                std::uniform_real_distribution<> dis_(-1.6, 1.6);
                float panMove = dis_(gen_);
                printf("...tiltMove : %.2f\n", tiltMove);
                printf("...panMove : %.2f\n", panMove);
                headMove(panMove, tiltMove);
                sleep(1);
        ...
        if (ballLost(20))
        {
            ...
            if (robotNumber == 1)
            {
                if (robot2FBall == 0 &&
                    ...
                    robot4FBall == 0 &&
                    robot5FBall == 0 )
                { // jika tidak ada robot yang melihat bola
                    normalSearchBall();
                } else 
                {
                    if (robot2FBall == 1)
                    ...
                    } else if (robot4FBall == 1)
                    {
                        HeadGrid = robot4GridPosition;
                    } else if (robot5FBall == 1)
                    {
    ...
    NodeStatus Support()
    {
        ...
        // printf("...targetPose : %f, %f\n", targetPoseX, targetPoseY);
        // motion("0");
        // new_out_grid(poseToGrid(targetPoseX, targetPoseY), 0, 0, true);
        // printf("...robot 2 grid pos = %d\n", robot2GridPosition);
        if (rst_support > 100)
        {
            // new_out_grid(robot2GridPosition-6, 0, 0, true);
            new_out_grid(poseToGrid(targetPoseX, targetPoseY), 0, 0, true);
        } else
        {
            motion("0");
    ...
    void resetVariable()
    {
        ...
        refreshMoveGrid();
        reset_velocity();
        cnt_path = cnt_path_ = cnt_get_data = state_move_grid = stateCondition = cnt_move_to_grid = rst_mov = state_gk = cnt_sit = rst_support = 0;
        Ball_X = Ball_Y = Goal_X = Goal_Y = -1;
        tiltReached = done_path_tracked = done_generated = Pickup = passed = homing = done_first_in_gk = doneGetData = false;
        ...
        rst_loc_kick_off = rst_dev_left = rst_dev_right = rst_mov_attack = 0;
    }
    ...
    void timer_callback()
    {
        ...
        robot_X = robot_Y = goal_L_pole_X = goal_L_pole_Y = goal_R_pole_Y = goal_R_pole_X = -1;
        ...
        for (const auto& bbox : msg->bounding_boxes)
        {
            ...
            switch (id_class)
            {
                ...
                case 11:
                    ...
                    continue;
                case 12:
                    // RCLCPP_INFO(this->get_logger(), "robot": (%d, %d)", x_center, y_center);
                    robot_X = x_center;
                    robot_Y = y_center;
                    continue;
                case 13:
                    // RCLCPP_INFO(this->get_logger(), "goal_L_pole: (%d, %d)", x_center, y_center);
                    goal_L_pole_X = x_center;
                    goal_L_pole_Y = y_center;
                    continue;
                case 14:
                    // RCLCPP_INFO(this->get_logger(), "goal_R_pole: (%d, %d)", x_center, y_center);
    ...
    void kick(int mode)
    {
        ...
        { // kiri
            ...
            {
                ballPositioning(-(pPanTendang - 0.15), pTiltTendang, ballPositioningSpeed, sudutTendang); // 0.15
            }
        }
        if (kanan)
        { // kanan
            ...
            {
                ballPositioning(pPanTendang, pTiltTendang, ballPositioningSpeed, sudutTendang); // 0.15
            }
        }
    }
    ...
    //trackBall();
		...
		if (robotDirection && headPan >= -0.4 && headPan <= 0.4) {
				if (ballPos) { //printf("ball pos left true\n");
                    ...
                    ballPositioning( -0.18, -0.45, ballPositioningSpeed, sudutTendang); //0.15
				}			
		} else {
    ...
    int poseToGrid(double posx, double posy)
    { //Konvert koordinat posisi robot (x,y) jadi Grid posisi robot
        int tempGridX, tempGridY, GridX, GridY;
        GridX = posx;
        GridY = posy;
        ...
        int grid_out = tempGridY + (6 * (tempGridX - 1));
        if (grid_out <= 1)
        {
            grid_out = 1;
        }
