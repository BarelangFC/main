...
public:
        ...
        : Node("main_strategy")
    {
        ...
        bounding_boxes_ = this->create_subscription<geometry_msgs::msg::PointStamped>(
            "obj_coor", 10,
...
private:
    ...
    void new_out_grid(int targetRobotGrid, int targetRobotGridOffsetX, int targetRobotGridOffsetY, bool isGoToGrid)
    {
        ...
        theta = atan2(targetRobotPos_Y - robotPos_Y, targetRobotPos_X - robotPos_X) * 180 / M_PI;
        ...
        if (isGoToGrid)
        {
            ...
            else
            {
                if (distance < 100)
                {
                    if (posRotateNew)   
                    {
                        if (targetRobotPos_X > robotPos_X) // target ada di depan
                        {
                            sagital = targetRobotPos_X - robotPos_X;
                            if (sagital <= 0)
                            {
                                walkSagital = 0.0;
                            } else 
                        ...
                        } else if (targetRobotPos_X < robotPos_X) // target ada di belakang
                        {
                            sagital = robotPos_X - targetRobotPos_X;
                            if (sagital <= 0)
                            {
                                walkSagital = 0.0;
                            } else 
                        ...
                        if (targetRobotPos_Y < robotPos_Y) // target ada di kiri
                        {
                            lateral = targetRobotPos_Y - robotPos_Y;
                            if (lateral >= 0)
                            {
                                walkLateral = 0.0;
                            }
                        ...
                        } else if (targetRobotPos_Y > robotPos_Y) // target ada di kanan
                        {
                            lateral = targetRobotPos_Y + robotPos_Y;
                            if (lateral >=0)
                            {
                        ...
                        printf("...jalan lateral !!! \n");
                        motion("0");
                        // homing = false;
                        // set_velocity(walkSagital, walkLateral, 0.0);
                    }
                    else
                    {
                        reset_velocity();
                ...
                } else
                {
                    if (cnt_move_to_grid > 10)
                    {
                        ...
                        } else
                        {
                            // if (msg_yaw > theta)
                            // {
                                // set_velocity(0.0, 0.0, 0.2);
                                jalanDirection(0.0, 0.0, theta);
                            // } else if (msg_yaw < theta) 
                            // {
                            //     set_velocity(0.0, 0.0, -0.2);
                            // }
                            printf("...rotate duls !!! \n");
    ...
    void resetVariable()
    {
        refreshMoveGrid();
        reset_velocity();
        state_move_grid = stateCondition = cnt_move_to_grid = rst_mov = state_gk = cnt_sit = 0;
        Ball_X = Ball_Y = Goal_X = Goal_Y = -1;
        Pickup = passed = homing = false;
        cnt_rst_1000 = sumWalkX = 0;
    ...
    double field_x = 0.0, field_y = 0.0, x_pos = 0.0, y_pos = 0.0;
    void new_mapping(double x_vel, double y_vel, int imu_yaw)
    {
        // Convert the IMU yaw from degrees to radians
        double imu_yaw_rad = imu_yaw * M_PI / 180.0;

        // Calculate the robot's new position based on its velocity and heading
        x_pos += x_vel * cos(imu_yaw_rad) + y_vel * sin(imu_yaw_rad);
        y_pos += x_vel * sin(imu_yaw_rad) + y_vel * cos(imu_yaw_rad);

        // Map the robot's position to the RoboCup field coordinates in cm
        field_x = x_pos * 100;
        field_y = y_pos * 100;
        // printf("field = %f, %f\n", field_x+10.0, field_y);
    }
    ...
    void set_velocity(double vWalkX, double vWalkY, double vWalkA)
    {
        ...
        if (vWalkX == 0.0 && vWalkY == 0.0 && vWalkA == 0.0)
        {
    ...
    void trackBall()
    {
        if (useVision)
        {
            ...
            { // printf("Tracking");
                ...
                errorPanB *= -1;
                errorTiltB *= -1;
                errorPanB *= (90 / (double)frame_X);  // pixel per msg_yaw
                errorTiltB *= (60 / (double)frame_Y); // pixel per msg_yaw
                // errorPanB *= (77.32 / (double)frame_X); // pixel per msg_yaw
                // errorTiltB *= (61.93 / (double)frame_Y); // pixel per msg_yaw
    ...
    { // 0 normal, 1 sambil belok
        ...
        else
        {
            ...
            PxMove = (kejarMax / (-1.6)) * (posTilt); //0.04-0.06
        ...
        { // Mode differential walking
            ...
            { // printf("AAAAAAAA\n");
                set_velocity(PxMove, 0.0, PaMove);
            }
            else
            { // printf("BBBBBBBB\n");
                set_velocity(0.0, 0.0, PaMove);
            }
        }
        else if (mode == 1)
        { // Mode omnidirectional walking
            if (errorfPan > -0.4 && errorfPan < 0.4)
            { // printf("CCCCCCCC\n");
                set_velocity(PxMove, PyMove, PaMove);
            }
            else
            { // printf("DDDDDDDD\n");
                set_velocity(0.0, 0.0, PaMove);
    ...
    void ballPositioningZero(double setPointX, double setPointY, double speed, int cudut)
    {
        ...
        { // belum sesuai
            ...
            else
            {
                ...
                {
                    PxMoveBallPosz = headTilt * (0.03 / -1.6); // 0.05
                }
    ...
    void kick(int mode)
    {
        ...
        { // kiri
            ...
            {
                ballPositioning(-(pPanTendang - 0.15), pTiltTendang, ballPositioningSpeed); // 0.15
            }
        }
        if (kanan)
        { // kanan
        ...
        if (headTilt >= cSekarang)
        {
            if (robotDirection && headPan >= -0.4 && headPan <= 0.4)
            {
                ...
                { // kiri
                    ...
                    {
                        ballPositioning(-(pPanOper - 20), pTiltOper, ballPositioningSpeed); // 0.15
                    }
                }
                else
                { // kanan
