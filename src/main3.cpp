...
public:
        ...
        : Node("main_strategy")
    {
        ...
        factory.registerSimpleCondition("isUpdateGrid", std::bind(&main_strategy::isUpdateGrid, this));
        factory.registerSimpleCondition("ishigh", std::bind(&main_strategy::ishigh, this));
        factory.registerSimpleCondition("ismedium", std::bind(&main_strategy::ismedium, this));
        factory.registerSimpleCondition("islow", std::bind(&main_strategy::islow, this));
        ...
        factory.registerSimpleAction("Banting", std::bind(&main_strategy::Banting, this));
        factory.registerSimpleAction("RotateKickOff", std::bind(&main_strategy::RotateKickOff, this));
...
private:
    NodeStatus isKillrun()
    {
        if (lastState != msg_kill)
        {
            if (msg_kill == 0)
            {
                stateCondition = stateConditionKickOff = firstStateCondition;
                play = true;
            }
            else
            {
        ...
        case 1: // follow Ball
            ...
            else
            {
                ...
                if (headTilt >= cAktif && headPan >= -0.4 && headPan <= 0.4)
                {
                    ...
                    if (!useSideKick)
                    {
                        if (robotPos_X < 0 || robotPos_Y > -100 && robotPos_Y < 100)
                        {
                            sudutTendang = theta;
                        } else 
                        {
    ...
    NodeStatus MoveToGrid()
    {
        ...
        {
            new_out_grid(26, 0, -50, true);
        }
        return NodeStatus::FAILURE;
    }

    bool updateGrid = false;
    NodeStatus isUpdateGrid()
    {
        if (updateGrid)
        {
            if (delay > 5)
            ...
            sendRobotCoordinationData(robotNumber, 1, 232, Grid, 1, 999, 88, 1, voltage);
            return NodeStatus::SUCCESS;
        } 
        return NodeStatus::FAILURE;
    }
    ...
    NodeStatus GetData()
    {
        ...
        {
            if(msg_strategy == 0){
                initialPos_X = convertGridX(53, 0);
                initialPos_Y = convertGridY(53, 0);
            }else if(msg_strategy == 1){
                initialPos_X = convertGridX(52, 0);
                initialPos_Y = convertGridY(52, 0);
            }else if(msg_strategy == 2){
                initialPos_X = convertGridX(51, 0);
                initialPos_Y = convertGridY(51, 0);
            }else if(msg_strategy == 3){
                initialPos_X = convertGridX(50, 0);
                initialPos_Y = convertGridY(50, 0);
            }
            if (cnt_sbr >= 4)
            {
                doneGetData = true;
    ...
    NodeStatus Communication()
    {
        ...
        {
            sendRobotCoordinationData(robotNumber, 1, stateCondition, Grid, 1, semeh, 88, 1, voltage);
            rst_head_grid = 0;
            return NodeStatus::SUCCESS;
        }
        return NodeStatus::FAILURE;
    ...
    NodeStatus Banting()
    {
        ...
        printf("case 0");
            ...
            }else
            {
                ...
                trackBall();
                if(tunggu > 25){
                    if(posTilt <= -1.70) // bola jauh
                    {
                        printf("trackball");
                    }else if( posTilt > -1.70 && posTilt < -1.2 ) //bola sedang
                    {
                        doneBanting = posRotateNew = false;
                        stateCondition = 1;
                    
                    }else if(posTilt >= -1.2) //bola dekat
                    {
                        delayWaitBall = 0;
                        stateCondition = 2;
                    }
    ...
    NodeStatus ismedium()
    {
        if(headTilt >= -1.75 && headTilt <= -1.15 )
        {
            return NodeStatus::SUCCESS;
        }
        return NodeStatus::FAILURE;
    ...
    NodeStatus Support()
    {
        ...
        if (rst_support > 100)
        {
            ...
            //     }
            // } else if (robotNumber == 3) // support kiri
            // {
            //     if (robot4FBall == 1) // attacker
            //     {
            //         support_grid = coordinates_to_grid(convertGridX(robot4GridPosition, -300), convertGridY(robot4GridPosition, -200));
            //     } else if (robot2FBall == 1) // supporter 
            //     {
    ...
    bool doneKickOff = false;
    int stateConditionKickOff = 0;
    NodeStatus RotateKickOff()
    {
        if (robotNumber != 1)
        {
            if (robot1State == 232)
        ...
        case 3: // BallPos -> Tendang
            ...
            else
            {
                ...
                // {
                    ...
                    {
                        // resetCase4();
                        // stateConditionKickOff = 4;
                        doneKickOff = true;
                        sendRobotCoordinationData(robotNumber, 1, 232, Grid, 1, semeh, 88, 1, voltage);
                        resetCase4();
                        stateConditionKickOff = 4;
                    }
                    else
    ...
    void resetVariable()
    {
        ...
        refreshMoveGrid();
        reset_velocity();
        cnt_path = cnt_path_ = state_move_grid = stateCondition = stateConditionKickOff = cnt_move_to_grid = rst_mov = state_gk = cnt_sit = rst_support = cnt_sbr = 0;
    ...
    void mapping(double arukuX, double arukuY)
    {
        ...
        if (arukuX > 0)
        {
            ...
            {
                sX = 6.28833 * arukuX / 0.05; //5.2
            }
            else if (arukuX > 0.05 && arukuX <= 0.06)
            {
                sX = 7.95666 * arukuX / 0.06; //5.7
            }
            else if (arukuX > 0.06 && arukuX <= 0.07)
            {
                sX = 8.0016666 * arukuX / 0.07; //6.9
            }
